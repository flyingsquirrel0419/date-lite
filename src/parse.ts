...
const PARSE_TOKENS_SORTED = Object.keys(PARSE_TOKEN_PATTERNS).sort(
...
export function parse(dateStr: string, pattern: string): Date {
	...
	let si = 0; // index in dateStr
	const pi = 0; // index in pattern
	...
	const segments: Array<
		{ type: "token"; token: string } | { type: "literal"; char: string }
	> = [];
